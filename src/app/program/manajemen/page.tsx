...
import { manajemen, kurikulumManajemen } from "@/data/manajemen";
    ...
    <div className="bg-white min-h-screen">
      ...
      <section className="max-w-5xl mx-auto px-4 py-16">
        ...
        <ul className="space-y-3">
          {manajemen.kurikulum.keunggulan.map((item, index) => (
        ...
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="bg-white rounded-xl shadow-md p-6">
            <p className="text-slate-700 leading-relaxed">
              {kurikulumManajemen.description}
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-[#8B3A3A]">
            <h3 className="text-xl font-semibold text-[#8B3A3A] mb-3">
              {manajemen.jenisProgram.reguler.title}
            </h3>
            <p className="text-slate-700">
              {manajemen.jenisProgram.reguler.description}
            </p>
          </div>
          <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-[#A64D4D]">
            <h3 className="text-xl font-semibold text-[#8B3A3A] mb-3">
              {manajemen.jenisProgram.karyawan.title}
            </h3>
            <p className="text-slate-700">
              {manajemen.jenisProgram.karyawan.description}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-slate-50 py-16">
        <div className="max-w-5xl mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-semibold text-[#8B3A3A] mb-6">
            Prospek Lulusan S1 Manajemen menjadi Ahli dan Praktis
          </h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {manajemen.prospekKarir.map((karir, index) => (
              <div
                key={index}
      ...
      {manajemen.pdfPath && (
        <section className="max-w-6xl mx-auto px-4 py-16">
          ...
          <div className="bg-slate-100 rounded-xl shadow-lg overflow-hidden">
            {!pdfError ? (
              <iframe
                src={`${manajemen.pdfPath}#toolbar=1&navpanes=1&scrollbar=1`}
