...
import { psikologi } from "@/data/psikologi";
    ...
    <div className="bg-white min-h-screen">
      ...
      <section className="max-w-5xl mx-auto px-4 py-16">
        ...
        <ul className="space-y-3">
          {psikologi.kurikulum.keunggulan.map((item, index) => (
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-[#8B3A3A]">
              <h3 className="text-xl font-semibold text-[#8B3A3A] mb-3">
                {psikologi.jenisProgram.reguler.title}
              </h3>
              <p className="text-slate-700">
                {psikologi.jenisProgram.reguler.description}
              </p>
            </div>
            <div className="bg-white rounded-xl shadow-md p-6 border-l-4 border-[#A64D4D]">
              <h3 className="text-xl font-semibold text-[#8B3A3A] mb-3">
                {psikologi.jenisProgram.karyawan.title}
              </h3>
              <p className="text-slate-700">
                {psikologi.jenisProgram.karyawan.description}
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-5xl mx-auto px-4 py-16">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {psikologi.prospekKarir.map((karir, index) => (
            <div
              key={index}
        ...
        <section className="bg-slate-50 py-16">
          <div className="max-w-6xl mx-auto px-4">
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden">
              {!pdfError ? (
                <iframe
                  src={`${psikologi.pdfPath}#toolbar=1&navpanes=1&scrollbar=1`}
                  className="w-full h-[800px]"
                  title="Jadwal Psikologi"
